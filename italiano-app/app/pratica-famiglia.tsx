...
export default function PraticaFamigliaScreen() {
    const router = useRouter();
    const colorScheme = useColorScheme();
    const theme = Colors[colorScheme ?? 'light'];

    const [famigliaExercises, setFamigliaExercises] = useState<Exercise[]>([]);
    ...
    useEffect(() => {
        const fetchExercises = async () => {
            try {
                const data = await getExercisesByLessonId('10');
                setFamigliaExercises(data);
            } catch (error) {
                console.error('Error fetching family exercises:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleOptionPress = (option: string) => {
        ...
        if (famigliaExercises.length === 0) return;

        setSelectedOption(option);
        const correct = option === famigliaExercises[currentIndex].correctAnswer;
        setIsCorrect(correct);

        if (correct) {
            setScore(score + 1);
            speak('Bene! ' + famigliaExercises[currentIndex].correctAnswer);
        } else {
            speak('No, riprova.');
        }
    };
    ...
    const nextExercise = () => {
        if (currentIndex < familiaExercises.length - 1) {
    ...
    const currentExercise = familiaExercises[currentIndex];
        ...
        return (
            <View style={[styles.container, { backgroundColor: theme.surface }]}>
                ...
                <View style={styles.finishedContent}>
                    ...
                    <Text style={[styles.finishedTitle, { color: theme.text }]}>¡Bellissimo!</Text>
                    <Text style={[styles.finishedSubtitle, { color: theme.muted }]}>
                        Has dominado la familia y los posesivos.
                    </Text>
                    <View style={[styles.scoreCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                        ...
                        <Text style={[styles.scoreValue, { color: theme.primary }]}>{score}/{famigliaExercises.length}</Text>
                    </View>
                    <TouchableOpacity
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.surface }]}>
            <Stack.Screen options={{ title: 'Práctica: Famiglia' }} />
                    ...
                    <View
                        style={[
                            styles.progressFill,
                            {
                                ...
                                width: `${((currentIndex + 1) / famigliaExercises.length) * 100}%`
                            }
                        ]}
                    />
                </View>
                <Text style={[styles.progressText, { color: theme.muted }]}>
                    {currentIndex + 1} de {famigliaExercises.length}
                </Text>
            </View>
