...
const ESERCIZI: Esercizio[] = [
    {
        ...
        answer: 'al',
        dialogue: [
            { person: 'A', text: 'Dove vai stasera?' },
            { person: 'B', text: 'Vado ', hasInput: true },
            { person: 'B', text: ' cinema con i miei amici.' }
        ],
        hint: 'Preposizione A + il (cinema).'
    },
    {
        id: 2,
        answer: 'alla',
        dialogue: [
            { person: 'A', text: 'Dov\'è Maria?' },
            { person: 'B', text: 'È ', hasInput: true },
            { person: 'B', text: ' stazione, aspetta il treno.' }
        ],
        hint: 'Preposizione A + la (stazione).'
    },
    {
        id: 3,
        answer: 'allo',
        dialogue: [
            { person: 'A', text: 'Andate a vedere la partita?' },
            { person: 'B', text: 'Sì, andiamo ', hasInput: true },
            { person: 'B', text: ' stadio adesso.' }
        ],
        hint: 'Preposizione A + lo (stadio).'
    },
    {
        id: 4,
        answer: 'nella',
        dialogue: [
            { person: 'A', text: 'Hai visto le mie chiavi?' },
            { person: 'B', text: 'Sì, sono ', hasInput: true },
            { person: 'B', text: ' borsa rossa in cucina.' }
        ],
        hint: 'Preposizione IN + la (borsa).'
    },
    {
        id: 5,
        answer: 'nel',
        dialogue: [
            { person: 'A', text: 'Dov\'è il latte?' },
            { person: 'B', text: 'È ', hasInput: true },
            { person: 'B', text: ' frigorifero.' }
        ],
        hint: 'Preposizione IN + il (frigorifero).'
    },
    {
        id: 6,
        answer: 'del',
        dialogue: [
            { person: 'A', text: 'Di chi è questo cane?' },
            { person: 'B', text: 'È il cane ', hasInput: true },
            { person: 'B', text: ' vicino di casa.' }
        ],
        hint: 'Preposizione DI + il (vicino).'
    },
    {
        id: 7,
        answer: 'dal',
        dialogue: [
            { person: 'A', text: 'Perché sei in ritardo?' },
            { person: 'B', text: 'Vengo ', hasInput: true },
            { person: 'B', text: ' medico, c\'era molta gente.' }
        ],
        hint: 'Preposizione DA + il (medico).'
    },
    {
        id: 8,
        answer: 'alle',
        dialogue: [
            { person: 'A', text: 'A che ora inizia la lezione?' },
            { person: 'B', text: 'Inizia ', hasInput: true },
            { person: 'B', text: ' nove meno un quarto.' }
        ],
        hint: 'Preposizione A + le (ore plurales).'
    },
    {
        id: 9,
        answer: 'negli',
        dialogue: [
            { person: 'A', text: 'Dove vive tuo fratello?' },
            { person: 'B', text: 'Vive ', hasInput: true },
            { person: 'B', text: ' Stati Uniti per lavoro.' }
        ],
        hint: 'Preposizione IN + gli (Stati Uniti).'
    },
    {
        id: 10,
        answer: 'della',
        dialogue: [
            { person: 'A', text: 'Ti piace questo profumo?' },
            { person: 'B', text: 'Sì, è il profumo ', hasInput: true },
            { person: 'B', text: ' primavera.' }
        ],
        hint: 'Preposizione DI + la (primavera).'
    }
];

export default function PraticaPreposizioniScreen() {
    ...
    if (hasStarted) {
        if (isCorrect) {
            ...
            if (userInput.length >= expectedAnswer.length - 1) {
                feedbackMessage = 'Controlla la combinazione.';
                feedbackColor = '#E65100';
                isWarning = true;
            } else {
                feedbackMessage = '📝 Scrivi la preposizione articolata…';
            ...
            <>
                ...
                <View style={[styles.container, { backgroundColor: theme.surface, justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
                    <View style={[styles.card, { backgroundColor: theme.card, width: '100%', alignItems: 'center', padding: 30 }]}>
                        ...
                        <Text style={[styles.title, { color: theme.text, textAlign: 'center' }]}>¡Maestro de Enlaces!</Text>
                        <Text style={[styles.subtitle, { color: theme.muted, textAlign: 'center', marginBottom: 30 }]}>
                            Hai imparato a unire preposizioni e articoli correttamente. ¡Ottimo lavoro!
                        </Text>
    ...
    return (
        <>
            <Stack.Screen options={{ title: 'Práctica: Preposizioni' }} />
            <ScrollView
