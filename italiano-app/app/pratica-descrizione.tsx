...
export default function PraticaDescrizioneScreen() {
    const router = useRouter();
    const colorScheme = useColorScheme();
    const theme = Colors[colorScheme ?? 'light'];

    const [descrizioneExercises, setDescrizioneExercises] = useState<Exercise[]>([]);
    ...
    useEffect(() => {
        const fetchExercises = async () => {
            try {
                const data = await getExercisesByLessonId('11');
                setDescrizioneExercises(data);
            } catch (error) {
                console.error('Error fetching description exercises:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleOptionPress = (option: string) => {
        if (isCorrect !== null || descrizioneExercises.length === 0) return;

        setSelectedOption(option);
        const correct = option === descrizioneExercises[currentIndex].correctAnswer;
        setIsCorrect(correct);

        if (correct) {
            setScore(score + 1);
            speak('Bene! ' + descrizioneExercises[currentIndex].correctAnswer);
        } else {
            speak('No, riprova.');
        }
    };
    ...
    const nextExercise = () => {
        if (currentIndex < descrizioneExercises.length - 1) {
    ...
    if (descrizioneExercises.length === 0) {
        return (
            <View style={[styles.container, { backgroundColor: theme.surface, justifyContent: 'center', alignItems: 'center' }]}>
    ...
    const currentExercise = descrizioneExercises[currentIndex];
        ...
        return (
            <View style={[styles.container, { backgroundColor: theme.surface }]}>
                ...
                <View style={styles.finishedContent}>
                    ...
                    <Text style={[styles.finishedTitle, { color: theme.text }]}>¡Ottimo lavoro!</Text>
                    <Text style={[styles.finishedSubtitle, { color: theme.muted }]}>
                        Has aprendido a describir personas y objetos.
                    </Text>
                    <View style={[styles.scoreCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                        ...
                        <Text style={[styles.scoreValue, { color: theme.primary }]}>{score}/{descrizioneExercises.length}</Text>
                    </View>
                    <TouchableOpacity
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.surface }]}>
            <Stack.Screen options={{ title: 'Práctica: Descrizione' }} />
                    ...
                    <View
                        style={[
                            styles.progressFill,
                            {
                                ...
                                width: `${((currentIndex + 1) / descrizioneExercises.length) * 100}%`
                            }
                        ]}
                    />
                </View>
                <Text style={[styles.progressText, { color: theme.muted }]}>
                    {currentIndex + 1} de {descrizioneExercises.length}
                </Text>
            </View>
