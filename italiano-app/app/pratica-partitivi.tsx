...
export default function PraticaPartitiviScreen() {
    ...
    useEffect(() => {
        const fetchExercises = async () => {
            try {
                ...
                const filtered = data.filter(ex => ex.subtopic === 'Artículos Partitivos');
                setPracticeExercises(filtered);
            } catch (error) {
                console.error('Error fetching partitive exercises:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return (
            <View style={[styles.container, { backgroundColor: theme.surface, justifyContent: 'center', alignItems: 'center' }]}>
                ...
                <Text style={{ color: theme.muted, marginTop: 10 }}>Caricamento esercizi...</Text>
            </View>
        );
    }
            ...
            <>
                ...
                <View style={[styles.container, { backgroundColor: theme.surface, justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
                    <View style={[styles.card, { backgroundColor: theme.card, width: '100%', alignItems: 'center', padding: 30, elevation: 10 }]}>
                        ...
                        <Text style={[styles.title, { color: theme.text, textAlign: 'center' }]}>Magnifico!</Text>
                        <Text style={[styles.subtitle, { color: theme.muted, textAlign: 'center', marginBottom: 30 }]}>
                            Hai padroneggiato gli articoli partitivi.
                        </Text>
    ...
    return (
        <>
            <Stack.Screen options={{
                title: 'Pratica: Articoli Partitivi',
