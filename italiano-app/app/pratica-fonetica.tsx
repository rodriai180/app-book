...
export default function PraticaFoneticaScreen() {
    const router = useRouter();
    const colorScheme = useColorScheme();
    const theme = Colors[colorScheme ?? 'light'];

    const [phoneticsExercises, setPhoneticsExercises] = useState<Exercise[]>([]);
    ...
    useEffect(() => {
        const fetchExercises = async () => {
            try {
                const data = await getExercisesByLessonId('0');
                setPhoneticsExercises(data);
            } catch (error) {
                console.error('Error fetching phonetics exercises:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleOptionPress = (option: string) => {
        if (isCorrect !== null || phoneticsExercises.length === 0) return;

        setSelectedOption(option);
        const correct = option === phoneticsExercises[currentIndex].correctAnswer;
        setIsCorrect(correct);

        if (correct) {
            setScore(score + 1);
            speak('Bene! ' + phoneticsExercises[currentIndex].correctAnswer);
        } else {
            speak('No, riprova.');
        }
    };
    ...
    const nextExercise = () => {
        if (currentIndex < phoneticsExercises.length - 1) {
    ...
    if (phoneticsExercises.length === 0) {
        return (
            <View style={[styles.container, { backgroundColor: theme.surface, justifyContent: 'center', alignItems: 'center' }]}>
    ...
    const currentExercise = phoneticsExercises[currentIndex];

    if (showFinished) {
        return (
            <View style={[styles.container, { backgroundColor: theme.surface }]}>
                <Stack.Screen options={{ title: 'Sesion Finalizada' }} />
                <View style={styles.finishedContent}>
                    <CheckCircle2 size={80} color={theme.primary} />
                    <Text style={[styles.finishedTitle, { color: theme.text }]}>¡Excelente!</Text>
                    <Text style={[styles.finishedSubtitle, { color: theme.muted }]}>
                        Has completado la práctica de Fonética.
                    </Text>
                    <View style={[styles.scoreCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                        ...
                        <Text style={[styles.scoreValue, { color: theme.primary }]}>{score}/{phoneticsExercises.length}</Text>
                    </View>
                    <TouchableOpacity
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.surface }]}>
            <Stack.Screen options={{ title: 'Práctica: Fonética' }} />
                    ...
                    <View
                        style={[
                            styles.progressFill,
                            {
                                ...
                                width: `${((currentIndex + 1) / phoneticsExercises.length) * 100}%`
                            }
                        ]}
                    />
                </View>
                <Text style={[styles.progressText, { color: theme.muted }]}>
                    {currentIndex + 1} de {phoneticsExercises.length}
                </Text>
            </View>
